...
int config_header_v1_v2_chk(SEC_IMG_HEADER *sec_hdr) {
    ...
    if (sec_hdr->img_len <= sec_hdr->s_off) {
        printf("[%s] IMG len (%d) <= sign off (%d)\n", MOD, sec_hdr->img_len, sec_hdr->s_off);
        printf("[%s] Invalid sign off\n", MOD);
        return -1;
    }

    /* check if sign length is greater than image length */
    if (sec_hdr->img_len < (sec_hdr->s_len + sec_hdr->s_off)) {
        printf("[%s] IMG len (%d) < sign len (%d) + sign off (%d)\n", MOD, sec_hdr->img_len, sec_hdr->s_len,
               sec_hdr->s_off);
        sec_hdr->s_len = sec_hdr->img_len - sec_hdr->s_off;
        printf("[%s] adjust sign len to (%d)\n", MOD, sec_hdr->s_len);
    }
...
int config_header_v3_chk(unsigned long long img_len) {
    ...
    DBG("[%s] sec_hdr->img_len = %llu\n", MOD, img_len);

    if (p_ext_cfg->verify_count == 0) {
        printf("[%s] Sign region count is zero, please check config file(v3)\n", MOD);
        return -1;
    }

    /* check for chunk size 0 */
    if (p_ext_cfg->chunk_size == 0) {
        printf("[%s] Chunk size is 0, truncate verify count to 1(v3)\n", MOD);
    ...
    for (i = p_ext_cfg->verify_count - 1; i >= 0; i--) {
        ...
        if (p_ext_cfg->verify_offset[i] > img_len) {
            printf("[%s] Remove config of offset %lld (0x%llx)(v3)\n", MOD,
    ...
    if (max_pos < region_end) {
        printf("[%s] The last region's original length is %lld (0x%llx)\n", MOD,
               p_ext_cfg->verify_length[i], p_ext_cfg->verify_length[i]);
        p_ext_cfg->verify_length[i] = max_pos - p_ext_cfg->verify_offset[i];
        printf("[%s] Adjust last region's original length to %lld (0x%llx)\n", MOD,
               p_ext_cfg->verify_length[i], p_ext_cfg->verify_length[i]);
    }

    /* check if need to sign for whole image */
    if (p_ext_cfg->verify_count == 1 && p_ext_cfg->verify_length[0] == 0) {
        printf("[%s] The sign length is zero, and sign offset is : %lld (0x%llx)\n", MOD, p_ext_cfg->verify_offset[0],
               p_ext_cfg->verify_offset[0]);
        p_ext_cfg->verify_length[0] = img_len - p_ext_cfg->verify_offset[0];
        printf("[%s] Set the sign length to rest whole image length : %lld (0x%llx)\n", MOD,
               p_ext_cfg->verify_length[0], p_ext_cfg->verify_length[0]);
    }
    ...
    for (i = 0; i < p_ext_cfg->verify_count; i++) {
        if (img_len < p_ext_cfg->verify_length[i]) {
            printf("[%s] Sign length exceeds image length(v3)\n", MOD);
            return -1;
        }

        if (p_ext_cfg->verify_length[i] == 0) {
            printf("[%s] Sign length can't be zero(v3)\n", MOD);
            return -1;
        }
        ...
        if (img_len < region_end) {
            printf("[%s] Sign region exceeds image length(v3)\n", MOD);
            return -1;
        }

        DBG("[%d] min_pos is %llu\n", i, min_pos);
        DBG("p_ext_cfg->verify_offset[%d] is %lld\n", i, p_ext_cfg->verify_offset[i]);
        DBG("[%d] region_end is %llu\n", i, region_end);
        ...
        if ((min_pos <= p_ext_cfg->verify_offset[i]) &&
            ...
            DBG("[%s] Sign region (%lld->%llu) ok(v3)\n", MOD,
                p_ext_cfg->verify_offset[i],
                region_end - 1);
        } else {
            printf("[%s] Sign region is overlap(v3)\n", MOD);
            return -1;
        }
    ...
    if (p_ext_cfg->verify_count == 0) {
        printf("[%s] Sign region count is zero, please check program(v3)\n", MOD);
        return -1;
    }
